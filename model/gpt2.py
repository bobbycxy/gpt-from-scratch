...
class GPT2(nn.Module):
    def __init__(self, cfg):
        ...
        self.blocks = nn.Sequential(*[Block(self.num_heads, self.n_embed, self.block_size, self.dropout) for _ in range(self.num_layers)])
    ...
    def forward(self, idx, targets = None):
        ...
        if targets == None:
            ...
            loss = F.cross_entropy(logits, targets) ## !! - create a function that will measure the losses simply by comparing the logits and the targets by the indexes
        return logits, loss
