...
wandb_run_name = 'multiheadattention-ffn-blocks-residconn-layernorm-dropout-bpe2048' # 'run' + str(time.time())
...
tokens = tokenizer_text.encode("utf-8") # raw bytes
tokens = list(map(int, tokens)) # convert to a list of integers in range 0..255 for convenience

vocab_size = 2048
num_merges = vocab_size - 256
ids = list(tokens) ## copy so that we won't destroy the original list
...
## prepare a decoder from the tokenizer
